...
public actor HostOrchestrator {
    ...
    private func executeAction(_ action: HostAction, parameters _: [String: String]) async throws -> HostActionResult {
        switch action.id {
        case "build":
        ...
        case "tools-check":
            try await executeToolsCheck()
        case "provision":
            try await executeProvision()
        case "tools-install":
            try await executeToolsInstall()
        case "tools-update":
            try await executeToolsUpdate()
        case "install-system-wide":
            try await executeInstallSystemWide()
        case "status":
    ...
    private func executeProvision() async throws -> HostActionResult {
        ...
        return HostActionResult(
            actionID: "provision",
            success: success,
            message: success ? "Host provisioned successfully" :
                "Provisioned with errors: \(errors.joined(separator: ", "))",
            details: details,
